...
export class SpamDetectionService {
  ...
  private async scanNote(
    ...
    if (spamScore >= 2) {
      // Threshold: 2 or more indicators
      this.detectedCount++;
      await this.handleSpam(note, channel, spamIndicators, spamScore);
    }
  }

  private checkUrlSpam(text: string): {
    isSpam: boolean;
    reason: string;
    score: number;
  } {
    ...
    if (urls.length >= 3) {
      return {
        isSpam: true,
        reason: `Multiple URLs (${urls.length})`,
        score: 2,
      };
    }
  ...
  private checkRepetition(text: string): {
    isSpam: boolean;
    reason: string;
    score: number;
  } {
    ...
    for (const [word, count] of wordCounts) {
      if (count >= 5) {
        return {
          isSpam: true,
          reason: `Repeated word: "${word}" (${count}x)`,
          score: 1,
        };
      }
    }
  ...
  private checkMentionSpam(text: string): {
    isSpam: boolean;
    reason: string;
    score: number;
  } {
  ...
  private async checkRateLimit(
    userId: string,
  ): Promise<{ isSpam: boolean; reason: string; score: number }> {
  ...
  private checkSpamKeywords(text: string): {
    ...
    return {
      ...
      reason:
        matchCount > 0
          ? `Spam keywords (${matchCount} pattern${matchCount > 1 ? "s" : ""})`
          : "",
      score,
    };
  }

  private checkTextEntropy(text: string): {
    isSpam: boolean;
    reason: string;
    score: number;
  } {
  ...
  private checkEmojiSpam(text: string): {
    isSpam: boolean;
    reason: string;
    score: number;
  } {
