...
class AutoModerator {
  ...
  async initialize(): Promise<void> {
    ...
    if (env.DISCORD_WEBHOOK_URL) {
      this.notifications.addNotifier(
        new DiscordNotifier(env.DISCORD_WEBHOOK_URL),
      );
    }
    ...
    console.log("someone rewrite this garbage");
    ...
    setInterval(() => {
      ...
      console.log(`\n📊 [DETECTION STATS]`);
      console.log(
        `   NSFW:     ${nsfwStats.scanned} scanned, ${nsfwStats.detected} detected`,
      );
      console.log(
        `   Virus:    ${virusStats.scanned} scanned, ${virusStats.detected} detected`,
      );
      console.log(
        `   Spam:     ${spamStats.scanned} scanned, ${spamStats.detected} detected`,
      );
      console.log(
        `   Phishing: ${phishingStats.scanned} scanned, ${phishingStats.detected} detected`,
      );
      console.log(`   Resolved: ${moderatorStats.resolved} issues resolved`);
      if (moderatorStats.lateResponses > 0) {
        console.log(
          `   ⚠️  Late:    ${moderatorStats.lateResponses} responses >24h`,
        );
      }
      if (moderatorStats.ignoredSelfActions > 0) {
        console.log(
          `   Ignored:  ${moderatorStats.ignoredSelfActions} self-actions`,
        );
      }
      if (moderatorStats.ignoredAutomodActions > 0) {
        console.log(
          `   Skipped:  ${moderatorStats.ignoredAutomodActions} automod actions`,
        );
      }
    }, 30000);
  }
